...
import type { ConflictResult, ConflictEntry } from '../types/conflict.js';
...
export function detectConflicts(
  ...
  const entries = category
    ? getEntriesByCategory(category as LearningCategory).filter(e => e.status === 'active')
    : getActiveEntries();
...
function checkContradiction(ruleA: string, ruleB: string): boolean {
  ...
  for (const [negPattern, posPattern] of negationPairs) {
    ...
    if (aHasNeg || bHasNeg) {
      ...
      if (overlapScore > 0.3) return true;
    }
  }
  ...
  if (dbInA.length > 0 && dbInB.length > 0) {
    ...
    if (sameSubject && dbInA[0] !== dbInB[0]) return true;
  }

  return false;
}

function findSubject(rule: string): string | null {
  const subjects = [
  ...
  for (const subject of subjects) {
    if (rule.includes(subject)) return subject;
  }
  return null;
}

function buildConflictReason(newRule: string, existingRule: string): string {
