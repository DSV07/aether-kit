...
export function createServer() {
    ...
    // Tool: save_learning
    server.tool('save_learning', 'Salva uma nova memória/aprendizado do usuário. Use quando o usuário expressar uma preferência persistente, correção recorrente ou padrão técnico.', {
    ...
    server.tool('search_learning', 'Busca memórias/aprendizados relevantes para uma tarefa. Use ANTES de iniciar qualquer tarefa para consultar preferências do usuário.', {
    ...
    // Tool: list_learnings
    server.tool('list_learnings', 'Lista memórias/aprendizados existentes com filtros opcionais.', {
        category: z.string().optional().describe('Filtrar por categoria'),
    ...
    server.tool('update_learning', 'Atualiza uma memória/aprendizado existente pelo ID.', {
    ...
    server.tool('delete_learning', 'Deleta (desativa) uma memória. Por padrão faz soft-delete (status → inactive). Use hardDelete=true para apagar permanentemente.', {
    ...
    server.tool('detect_conflicts', 'Detecta conflitos entre uma nova regra e memórias existentes.', {
    ...
    server.tool('promote_to_skill', 'Gera uma Skill (.md) a partir de memórias consolidadas de uma ou mais categorias.', {
    ...
    server.tool('classify_instruction', 'Classifica se uma instrução do usuário deve virar memória persistente. Analisa palavras-chave, reutilizabilidade e escopo.', {
    ...
    server.tool('summarize_memory', 'Gera um resumo estatístico das memórias ativas: total por categoria, escopo, status e top regras.', {
    ...
    // Tool: export_memory
    server.tool('export_memory', 'Exporta todas as memórias para um arquivo JSON para backup.', {
    ...
    // Tool: import_memory
    server.tool('import_memory', 'Importa memórias de um arquivo JSON de backup.', {
