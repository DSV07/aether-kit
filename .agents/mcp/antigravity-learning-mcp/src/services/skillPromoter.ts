...
import type { SkillPromotionInput, SkillPromotionResult, SkillDefinition } from '../types/skill.js';
import { getActiveEntries, getEntriesByCategory } from '../storage/memoryStore.js';
import { getSkillsDir, getGeneratedSkillsDir, ensureDir, writeFileSafe } from '../utils/fileSystem.js';
import { detectConflicts } from './conflictDetector.js';

export function promoteToSkill(input: SkillPromotionInput): SkillPromotionResult {
  // Gather entries
  ...
  if (input.ruleIds && input.ruleIds.length > 0) {
    ...
    for (const cat of input.categories) {
      entries.push(...getEntriesByCategory(cat as LearningCategory).filter(e => e.status === 'active'));
    }
  }
  ...
  const nonConflicting: LearningEntry[] = [];
...
function buildSkillContent(name: string, entries: LearningEntry[]): string {
  ...
  const rulesByCategory = new Map<string, LearningEntry[]>();
